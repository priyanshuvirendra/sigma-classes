...
function CourseDetails() {
      ...
      try {
        ...
        if (enrollmentResponse.ok) {
          ...
          const currentEnrollment = Array.isArray(enrollmentData)
            ? enrollmentData.find(
              (enrollment) =>
                String(enrollment.course?.id) === String(courseId)
            )
            : null;
  ...
  const handleEnrollmentRequest = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.message || "Unable to submit enrollment request."
        );
      }
      ...
      setEnrollmentMessage(
        error.message || "Unable to submit enrollment request."
      );
...
<div className="overview-item">
  ...
  >
    Enroll Now
              ...
              {isStudent && enrollmentStatus === "PENDING" && (
                <div className="enrollment-status-message">
                  Enrollment request pending admin approval.
                </div>
              )}
              ...
              {isStudent &&
                ["INACTIVE", "COMPLETED", "CANCELLED"].includes(
                  enrollmentStatus
                ) && (
                  <button
                    type="button"
              ...
              <a
                href={`https://wa.me/9471268826?text=${encodeURIComponent(whatsappMessage)}`}
